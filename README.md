...
**Progressive Tool Discovery** solves this by loading tools on-demand. Instead of listing everything, the server provides a **Gateway** of meta-tools:

| Meta-Tool | Description |
|-----------|-------------|
| `search_catalog` | Finds tools in the 100+ tool catalog matching the user's current intent |
| `load_tool` | Dynamically activates a tool, adding it to the LLM's active context |
| `unload_tool` | Removes a tool from active context to prevent bloat after a task is done |

SkipperMCP implements this pattern while also offering **Resources** (static/dynamic context) and **Prompts** (pre-defined templates) to provide a complete maritime assistance experience.
...
- `search_catalog` — Search the maritime tool catalog to find relevant capabilities
- `load_tool` — Activate a specific tool from the catalog
- `unload_tool` — Clean up the context window by removing an active tool

### Catalog Tools (Loaded Dynamically)
...
**Resources & Prompts**
- `vessel://status/logs` — (Resource) Recent maintenance and status logs
- `marina://{marina_id}/regulations` — (Resource Template) Local harbor rules
- `arrival-briefing` — (Prompt) Guided arrival procedure for harbor entry
