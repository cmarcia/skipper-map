...
// GetResource handles reading of vessel logs and marina regulations.
func (h *SkipperHandler) GetResource(ctx context.Context, req *mcp.ReadResourceRequest) (*mcp.ReadResourceResult, error) {
	uri := req.Params.URI
	var content string

	switch {
	case uri == "vessel://status/logs":
		content = "VESSEL LOGBOOK - SV DISCOVERY\n2026-06-05: Engine oil changed. Hours: 1240.\n2026-06-06: Fuel topped off (150 gal). Water tanks sanitized.\n2026-06-07: Navigation lights tested - OK. Bilge clear."
	case strings.HasPrefix(uri, "marina://") && strings.HasSuffix(uri, "/regulations"):
		marinaID := strings.TrimSuffix(strings.TrimPrefix(uri, "marina://"), "/regulations")
		content = fmt.Sprintf("REGULATIONS FOR MARINA: %s\n1. Speed limit 5kts within breakwater.\n2. Contact Dockmaster on VHF Ch 71 for slip assignment.\n3. No discharge of black water within harbor limits.\n4. Rafting only permitted with prior approval.", strings.ToUpper(marinaID))
	default:
...
// GetPrompt provides pre-defined interaction templates.
func (h *SkipperHandler) GetPrompt(ctx context.Context, req *mcp.GetPromptRequest) (*mcp.GetPromptResult, error) {
	if req.Params.Name != "arrival-briefing" {
		return nil, fmt.Errorf("prompt not found: %s", req.Params.Name)
	}

	marina := req.Params.Arguments["marina_name"]
	if marina == "" {
		marina = "Destination Marina"
	}
	vessel := req.Params.Arguments["vessel_name"]
	if vessel == "" {
		vessel = "the vessel"
	}

	return &mcp.GetPromptResult{
		Description: "Generates a comprehensive arrival briefing for a harbor entry.",
		Messages: []*mcp.PromptMessage{
			{
				Role: mcp.Role("assistant"),
				Content: &mcp.TextContent{
					Text: fmt.Sprintf("I will help you prepare the arrival briefing for %s entering %s. I will examine the vessel's recent logs and the local harbor regulations to ensure a safe and compliant entry.", vessel, marina),
				},
			},
			{
				Role: mcp.Role("user"),
				Content: &mcp.TextContent{
					Text: fmt.Sprintf("Please provide the arrival briefing for %s at %s. Use the 'vessel://status/logs' resource and the 'marina://%s/regulations' resource.", vessel, marina, strings.ToLower(marina)),
				},
			},
		},
	}, nil
